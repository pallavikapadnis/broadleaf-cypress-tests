...
describe('Broadleaf Commerce Demo Add to Cart and Checkout', () => {
    it('should add the product to the cart and proceed to checkout', () => {
        ...
        // Verify the product is added and proceed to the cart
        cy.get('.js-inCartLinkContainer').should('be.visible');
        cy.get('.js-inCartLinkContainer a').should('contain', 'View in Cart');
        cy.get('a.btn.btn-primary.goto-full-cart').click();
        cy.url().should('include', '/cart');
        ...
        // Wait for the "Submit Checkout Stage" button to be visible before clicking
        cy.get('a.js-submitCheckoutStage').should('be.visible').click();

        // Select the fulfillment option and proceed
        cy.get('input#fulfillmentOptionId2').should('be.visible').check({ force: true });
        cy.get('a.js-submitCheckoutStage').should('be.visible').click();

        // Wait for the COD payment option to be visible
        cy.get('a[data-toggle="pill"][href="#COD"]').should('be.visible').click();

        // Complete the payment process and finalize checkout
